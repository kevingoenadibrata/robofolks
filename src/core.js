/* robofolks: seeded pixel-robot avatars.
   ...
   in a 4-frame walk cycle (folkWalkFrame), and painted onto a canvas for a
   sprite sheet (paintFolkSheet).

   Animations advance one tick every FOLK_TICK_MS. */
export const FOLK_TICK_MS = 160;
export const FOLK_W = 32;
export const FOLK_H = 16;
// [highlight, base, shade] -- lit from the top left.
export const FOLK_BODIES = [
...
export const FOLK_BODY_NAMES = ['Orange', 'Pink', 'Blue', 'Cream', 'Red', 'Lime', 'Dark gray'];
...
export const FOLK_VIEWS = ['front', 'back', 'left', 'right'];
export const FOLK_STATES = ['active', 'needs', 'waiting'];
...
export const FOLK_WALK_FRAMES = 4;
...
export function folkTraits(seed, overrides = null) {
  const traits = seededTraits(seed);
  if (!overrides) return traits;
  const paint = FOLK_BODY_NAMES.indexOf(overrides.body);
  if (paint >= 0) traits.body = FOLK_BODIES[paint];
  for (const part of Object.keys(FOLK_PARTS)) {
    if (FOLK_PARTS[part].includes(overrides[part])) traits[part] = overrides[part];
  }
  return traits;
}
...
function seededTraits(seed) {
  ...
  const body = pick(FOLK_BODIES);
  const antenna = pick(FOLK_PARTS.antenna);
  const ears = pick(FOLK_PARTS.ears);
  r(); // this draw used to pick the eye shape; kept so existing robots keep their chest
  const chest = pick(FOLK_PARTS.chest);
  const phase = Math.floor(r() * 40); // so a row of folks doesn't move in lockstep
  const build = pick(FOLK_PARTS.build); // drawn last so adding it didn't change the other parts
...
function folkPieces(traits) {
  const g = Array.from({ length: FOLK_H }, () => Array(FOLK_W).fill('.'));
  const set = (y, x, c) => { if (y >= 0 && y < FOLK_H && x >= 0 && x < FOLK_W) g[y][x] = c; };
  const row = (y, x0, x1, c) => { for (let x = x0; x <= x1; x++) set(y, x, c); };
  const mirror = (x) => FOLK_W - 1 - x;
  ...
  const ballSlide = (n, seam = [8, 23], stride = 1) => {
    ...
    for (let cx = first; cx <= FOLK_W + 6; cx += 12) stamp(MARKING[traits.chest] || MARKING.core, cx, 12);
  };
...
export function folkFrame(traits, state, tick) {
  const t = tick + traits.phase;
  const p = folkPieces(traits);
  p.head('front');
  p.torso('front');
...
export function folkWalkFrame(traits, view, frame) {
  const f = ((frame % FOLK_WALK_FRAMES) + FOLK_WALK_FRAMES) % FOLK_WALK_FRAMES;
  const p = folkPieces(traits);
...
export function folkColors(traits, state, light, lit) {
...
export function eachFolkRect(frame, colors, fn) {
...
export const FOLK_VIEWBOX = [-2, -10, 36, 44];

/** A glyph's pixels: fn(x, y, width, height) in grid units. Glyphs sit on the
 *  grid (row -1 is just above the antenna) and don't bob with the robot. */
export function eachGlyphRect(glyph, fn) {
  FOLK_GLYPHS[glyph.ch].forEach((line, r) => [...line].forEach((c, dc) => {
    if (c === '#') fn(glyph.col + dc, (glyph.row + r) * 2, 1, 2);
  }));
}

/** A frame as SVG markup: the robot's rects (bobbing by frame.dy) and the
 *  glyphs beside it. Internal; shared with animated.js. */
export function frameShapes(traits, state, frame) {
  const colors = folkColors(traits, state, frame.light, frame.lit);
  const rects = [];
  eachFolkRect(frame, colors, (x, y, w, h, fill) => {
  ...
  const glyphs = frame.glyphs.map((gl) => {
    const cells = [];
    eachGlyphRect(gl, (x, y, w, h) => cells.push(`<rect x="${x}" y="${y}" width="${w + 0.02}" height="${h + 0.02}"/>`));
...
function frameSvg(traits, state, frame) {
  const { rects, glyphs } = frameShapes(traits, state, frame);
  return `<svg viewBox="${FOLK_VIEWBOX.join(' ')}" shape-rendering="crispEdges">${rects.join('')}${glyphs.join('')}</svg>`;
}

export function folkSvg(traits, state, tick) {
  return frameSvg(traits, state, folkFrame(traits, state, tick));
}

export function folkWalkSvg(traits, view, frame) {
  return frameSvg(traits, 'active', folkWalkFrame(traits, view, frame));
}
...
export function paintFolkSheet(canvas, traits, scale = 4) {
  const cell = 34;
  canvas.width = FOLK_WALK_FRAMES * cell * scale;
  canvas.height = FOLK_VIEWS.length * cell * scale;
  const ctx = canvas.getContext('2d');
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  FOLK_VIEWS.forEach((view, r) => {
    for (let f = 0; f < FOLK_WALK_FRAMES; f++) {
      const frame = folkWalkFrame(traits, view, f);
      const colors = folkColors(traits, 'active', frame.light, frame.lit);
      const ox = f * cell + 1;
      const oy = r * cell + 1;
      eachFolkRect(frame, colors, (x, y, w, h, fill) => {
        ctx.fillStyle = fill;
